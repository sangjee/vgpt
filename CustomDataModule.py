...
def get_transforms(mode="train"):
    if mode == "train":
        ...
        return A.Compose(
            [
                A.Resize(224, 224, always_apply=True),
                A.Normalize(max_pixel_value=255.0, always_apply=True),A.Normalize(max_pixel_value=255.0, always_apply=True), # Use the mean value and std value calculated in imagenet, which is commonly used => mean=(0.485, 0.456, 0.406),std=(0.229, 0.224, 0.225)
            ]
        )